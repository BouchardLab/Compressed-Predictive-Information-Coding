from PFPC import PastFutureDataset, train_PFPC, DCA_init
...
def plot_latent_trials(X_dynamics, X_pca_trans, X_dca_trans, X_pfpc_trans, num_vis, snr_val=None, save_dir=None,
                       plot_lorenz_func=plot_lorenz_3d_colored, saved_root=None, show_title=False, max_2norm=3.2):
    ...
    max_2norm_pfpc = np.max(np.linalg.norm(X_pfpc_trans[:num_vis] - X_dynamics[:num_vis], axis=1))
    print("snr_val:{}, max R2:{}".format(snr_val, np.max((max_2norm_dca, max_2norm_pfpc))))
    ...
    fig = plt.figure()
    ax = plt.axes(projection='3d')
    p = plot_lorenz_func(ax, X_pfpc_trans[:num_vis], X_dynamics[:num_vis], linewidth_3d, max_2norm=max_2norm)
    if show_title:
        plt.title("Embedded dynamics by CPIC, SNR={}".format(snr_val))
    if saved_root is None:
        plt.show()
    else:
        plt.savefig(save_dir + "/pfpc_dynamics_snr_{}.png".format(snr_val))
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default="lorenz_stochastic_infonce_exploration")
    ...
    for snr_val, X_pca_trans, X_dca_trans, X_noisy in zip(snr_vals, X_pca_trans_dset, X_dca_trans_dset, X_noisy_dset):
        ...
        if do_dca_init:
            ...
            init_weights = None
        PFPC = train_PFPC(beta, xdim, ydim, mi_params, critic_params, baseline_params, num_epochs, train_dataloader,
                          signiture=args.config, deterministic=deterministic, init_weights=init_weights, lr=lr,
                          num_early_stop=num_early_stop, device=device).to(device)
        encoded_mean = PFPC.encode(torch.from_numpy(X_noisy).to(device))
        X_pfpc_trans = aligned_encoded_mean = linear_alignment(encoded_mean.cpu().detach().numpy(), X_dynamics)

        R2_PCA = compute_R2(X_pca_trans, X_dynamics)
        R2_DCA = compute_R2(X_dca_trans, X_dynamics)
        R2_PFPC = compute_R2(aligned_encoded_mean, X_dynamics)
        print("R2(PCA): {}, R2(DCA): {}, R2(PFPC): {}".format(R2_PCA, R2_DCA, R2_PFPC))
        R2_metrics.append([R2_PCA, R2_DCA, R2_PFPC])
        inferred_pfpc_trials.append(X_pfpc_trans)
        if do_vis_latent_trials:
            plot_latent_trials(X_dynamics, X_pca_trans, X_dca_trans, X_pfpc_trans, num_vis, snr_val=snr_val, save_dir=saved_root)
    R2_metrics = np.stack(R2_metrics)
    if args.seed is not None:
        with open(saved_root + "/latent_R2_seed{}.pkl".format(args.seed), "wb") as f:
            pickle.dump({"R2_metrics": R2_metrics, "snr_vals": snr_vals}, f)
        with open(saved_root + "/inferred_trials_seed{}.pkl".format(args.seed), "wb") as f:
            pickle.dump({"inferred_pfpc_trials": inferred_pfpc_trials, "snr_vals": snr_vals}, f)
    else:
