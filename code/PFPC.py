...
class PFPC(nn.Module):
    def __init__(self, xdim, ydim, mi_params, critic_params, baseline_params, T=4, beta=1e-3, beta1=1, beta2=1, hidden_dim=256,
                 deterministic=False, init_weights=None, device='cuda:0', critic_params_YX=None):
        ...
        # initialize critic and baseline for I_XY, I_YY
    ...
    def forward(self, x_past, x_future, debug=False):
        ...
        if self.deterministic:
            ...
            I_XY_bound = estimate_mutual_information(self.mi_params['estimator_compress'], x_past, encoded_past_reshaped, decoder=self.encoder, device=self.device)
        I_YY_bound = estimate_mutual_information(self.mi_params['estimator_predictive'], encoded_past_reshaped,
                                                 encoded_future_reshaped, critic_fn=self.critic, baseline_fn=self.baseline, device=self.device)
        if self.beta2 > 0:
            ...
            L = self.beta * I_XY_bound - self.beta1 * I_YY_bound - self.beta2 * I_YX_bound
        else:
            L = self.beta * I_XY_bound - self.beta1 * I_YY_bound
        # print(debug)
        if debug:
...
def train_PFPC(beta, xdim, ydim, mi_params, critic_params, baseline_params, num_epochs, train_loader, T=4, signiture=22,
               deterministic=False, init_weights=None, num_early_stop=0, device="cuda:0", lr=1e-4, beta1=1, beta2=0, critic_params_YX=None):
    model = PFPC(xdim, ydim, mi_params, critic_params, baseline_params, T=T, beta=beta, beta1=beta1, beta2=beta2,
