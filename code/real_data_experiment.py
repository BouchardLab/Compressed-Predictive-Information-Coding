...
def run_analysis_pfpc(X, Y, T_pi_vals, dim_vals, offset_vals, num_cv_folds, decoding_window,
    ...
    :param X:
    :param Y:
    :param T_pi_vals: window size for DCA and PFPC
    ...
    for X_train, X_test, Y_train, Y_test, fold_idx in cv:
        ...
        for dim_idx in range(len(dim_vals)):
            ...
            for T_pi_idx in range(len(T_pi_vals)):
                ...
                # encode train data and test data via PFPC
                ...
                for offset_idx in range(len(offset_vals)):
                    ...
                    r2_pfpc = linear_decode_r2(X_train_pfpc, Y_train_ctd, X_test_pfpc, Y_test_ctd, decoding_window=decoding_window, offset=offset)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='PFPC for real data.')
    parser.add_argument('--config', type=str, default="temp_stochastic_infonce")
    parser.add_argument('--model', type=str, default="PFPC")
    args = parser.parse_args()
    if args.model == "PFPC":
        from PFPC import PastFutureDataset, train_PFPC, DCA_init, Polynomial_expand
    if args.model == "PFPC_RC":
        from PFPC_RC import PastFutureDataset, train_PFPC, DCA_init, Polynomial_expand
    if args.config == 'm1_stochastic_infonce':
        ...
        # ydims = np.array([3, 4, 5, 6]).astype(int)
        ydims = np.array([5]).astype(int)
    else:
    ...
    for ydim in ydims:
        ...
        result = run_analysis_pfpc(X, Y, T_pi_vals, dim_vals=[ydim], offset_vals=offsets, num_cv_folds=n_cv, decoding_window=win,
